...
def plt_ramachandran(data, coordinates, limits=None, name=None, labels=None):
    ...
    major_locator_x = ticker.MultipleLocator(
        base=num_ticks_x)  # Set major tick every 30 degrees
    major_locator_y = ticker.MultipleLocator(
        base=num_ticks_y)  # Set major tick every 30 degrees
    ...
    minor_locator_x = ticker.MultipleLocator(
        base=num_ticks_x/4)  # Set minor tick every 10 degrees
    minor_locator_y = ticker.MultipleLocator(
        base=num_ticks_y/4)  # Set minor tick every 10 degrees
...
def plt_distribution(data_dict: dict, coordinates_dict,  limits=None, name=None, labels=None):
    # import necessary libraries and modules
    import matplotlib.pyplot as plt
    import matplotlib.ticker as ticker
    import matplotlib.font_manager as font_manager
    ...
    for i, var1_name in enumerate(var1_names):
        ...
        for j, var2_name in enumerate(var2_names):
            ...
            ax.plot(data_dict[f"{var1_name} {var2_name} dens"],
                    coordinates_dict[f"{var1_name} {var2_name} coords"][0],
                    color=colors[j],
                    linewidth=0.75,
                    label=f"{var2_name - 1}-{var2_name}"
                    )
    ...
    plt.subplots_adjust(left=0.15, right=0.95, top=0.80, bottom=0.15, wspace=0.5, hspace=0.5)
